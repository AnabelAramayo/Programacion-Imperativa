...
//B
  listarDepartamentos: function (arrayDepartamentos) {
    for (let i=0; i<arrayDepartamentos.length;i++){
      let variable = arrayDepartamentos[i]
      /*console.log (
        "El id: " + variable.id +
        ", precio $" + variable.precioAlquiler +
        (variable.disponible === true ? ". Disponible " : ". Alquilado ") +
        variable.cantidadHabitacion + " habitaciones" +
        ", máximo de: " + variable.cantidadPersonas + " personas" +
        (variable.aceptaMascotas === true ? ", Acepta mascotas. " : ", No acepta mascotas. ") +
        variable.propietarios + " son los propietarios.")*/
        console.log (`El id: ${variable.id}, precio $${variable.precioAlquiler} ${(variable.disponible === true ? ". Disponible " : ". Alquilado ")} ${variable.cantidadHabitacion} habitaciones ,máximo de: ${variable.cantidadPersonas} personas 
    ${(variable.aceptaMascotas === true ? ", Acepta mascotas. " : ", No acepta mascotas. ")} 
    ${variable.propietarios} son los proopietarios.`)
    }  
  },
// C
  departamentosDisponibles : function (arrayDepartamentos) {
    ...
    for (let i = 0; i<arrayDepartamentos.length;i++){
      if (arrayDepartamentos[i].disponible==true){
        guardandoDepto.push(arrayDepartamentos[i])
      } 
    } return guardandoDepto
  },
  ...
  buscarPorId : function (arrayDepartamentos, id) {
    ...
    for ( let i=0; i<arrayDepartamentos.length;i++){
      if (arrayDepartamentos[i].id == id){
        deptoEncontrado.push(arrayDepartamentos[i]);
      }
...
console.log(inmobiliaria.listarDepartamentos(inmobiliaria.departamentosDisponibles(departamentos)));

console.log(v, oo + " D. buscarPorId");
// Ejecución aquí
console.log(inmobiliaria.buscarPorId(departamentos, 5));
console.log(inmobiliaria.listarDepartamentos(inmobiliaria.buscarPorId(departamentos, 5)));
